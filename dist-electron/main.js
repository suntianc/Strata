...
app.whenReady().then(async () => {
  try {
    const { initPGlite } = await import("./pg-C--6FeAb.js");
    const { initLanceDB } = await import("./vector-BPcG6pNR.js");
    const { initKuzuDB } = await import("./graph-DlPa-fCr.js");
...
async function registerIPCHandlers() {
  ...
  const { MessageService } = await import("./messageService-A3hRoZEF.js");
  const { TaskService } = await import("./taskService-D5flz0dx.js");
  const { IngestionService } = await import("./ingestion-BJ8Rv1HB.js");
  const { RetrievalService } = await import("./retrieval-CPSXKZTw.js");
