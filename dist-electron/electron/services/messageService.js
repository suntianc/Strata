...
export class MessageService {
    ...
    static async create(payload) {
        ...
        await execute(`INSERT INTO messages (id, task_id, content, version, is_archived, created_at, updated_at)
       VALUES ($1, $2, $3, 1, FALSE, NOW(), NOW())`, [messageId, taskId || null, content]);
        // 2. Insert tags
        for (const tag of tags) {
            await execute(`INSERT INTO message_tags (message_id, tag) VALUES ($1, $2)
         ON CONFLICT DO NOTHING`, [messageId, tag]);
        }
        ...
        for (const filePath of files) {
            ...
            await execute(`INSERT INTO attachments (id, message_id, file_path, file_type, file_name)
         VALUES ($1, $2, $3, $4, $5)`, [attachmentId, messageId, filePath, fileType, fileName]);
            attachments.push({
                id: attachmentId,
                type: fileType,
                name: fileName,
        ...
        const dbMessage = await queryOne('SELECT * FROM messages WHERE id = $1', [messageId]);
    ...
    static async update(payload) {
       ...
       WHERE id = $2`, [content, id]);
        ...
        const dbMessage = await queryOne('SELECT * FROM messages WHERE id = $1', [id]);
    ...
    static async archive(id) {
        await execute('UPDATE messages SET is_archived = TRUE WHERE id = $1', [id]);
    }
    /**
    ...
    static async move(payload) {
        ...
        // Update PGlite
        await execute('UPDATE messages SET task_id = $1 WHERE id = $2', [targetTaskId, messageId]);
        // Update graph
        await linkMessageToTask(messageId, targetTaskId);
    }
    // ========== Helper Methods ==========
    static async getAttachments(messageId) {
        const attachments = await query('SELECT * FROM attachments WHERE message_id = $1', [messageId]);
    ...
    static async getTags(messageId) {
        const rows = await query('SELECT tag FROM message_tags WHERE message_id = $1', [messageId]);
